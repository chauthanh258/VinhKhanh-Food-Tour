...
export default function Analytics() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 p-8 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">QR Scans by POI</h2>
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-orange-500 rounded-full"></div>
                <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Scans</span>
              </div>
            </div>
          </div>
            ...
            {qrStats?.byPoi && qrStats.byPoi.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={qrStats.byPoi}>
                  ...
                  <XAxis dataKey="poiName" axisLine={false} tickLine={false} tick={{ fill: "#9ca3af", fontSize: 12 }} dy={10} />
        ...
        <div className="bg-white dark:bg-gray-900 p-8 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-8">Scan Source Distribution</h2>
          <div className="h-[300px] w-full relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{qrStats?.totalScans || 0}</p>
              <p className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Total Scans</p>
            </div>
          </div>
              ...
              <div key={`${item.source}-${index}`} className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <span className="text-sm font-medium text-gray-600 dark:text-gray-400 truncate">
                    {item.source === 'app' ? 'In-App Scanner' : 'External Camera'}
                  </span>
                </div>
      ...
      {/* POIs vs Menu Items Row (Existing) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 p-8 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
          <div className="flex items-center justify-between mb-8">
